...
count_in_location<-function(num_place){
  num_in_state<-sum(str_detect(locations, num_place))
  result<-paste("There were", num_in_state,"protests in",num_place)
  return(result)
}
...
count_on_date<-function(num_time){
  num_in_city<-sum(str_detect(dates, num_time))
  result<-paste("there were", num_in_city, "protest in", num_time)
  return(result)
}
